...
public class SaveLastVisitedPageMiddleware
{
    private readonly RequestDelegate _next;
    private readonly ILogger<SaveLastVisitedPageMiddleware> _logger;

    public SaveLastVisitedPageMiddleware(RequestDelegate next, ILogger<SaveLastVisitedPageMiddleware> logger)
    {
        _next = next;
        _logger = logger;
    }
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        if (method == "GET" && !string.IsNullOrEmpty(requestPath))
        {
            ...
            // Paths to be excluded from redirection
